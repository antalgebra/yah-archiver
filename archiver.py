...
def initialize_database(path: Path) -> sqlite3.Connection:
    ...
    connection.executescript(
        ...
        CREATE INDEX IF NOT EXISTS message_presence_current_folder_idx
            ON message_presence(folder, uidvalidity, uid)
            WHERE present = 1;
        ...
        CREATE INDEX IF NOT EXISTS audit_events_unalerted_idx
            ON audit_events(observed_at, event_key)
...
def archived_message_sha256s(
    database: sqlite3.Connection,
    folder: str,
    uidvalidity: int,
    uids: tuple[int, ...],
) -> dict[int, str]:
    """Load archived hashes in bounded batches instead of an N+1 query loop."""

    result: dict[int, str] = {}
    for offset in range(0, len(uids), SQLITE_UID_BATCH_SIZE):
        batch = uids[offset : offset + SQLITE_UID_BATCH_SIZE]
        placeholders = ",".join("?" for _uid in batch)
        rows = database.execute(
            "SELECT uid, sha256 FROM imap_messages "
            f"WHERE folder = ? AND uidvalidity = ? AND uid IN ({placeholders})",
            (folder, uidvalidity, *batch),
        ).fetchall()
        result.update(
            (int(row["uid"]), str(row["sha256"])) for row in rows
        )
    return result


def content_is_junk_suppressed(
    database: sqlite3.Connection, sha256: str | None
) -> bool:
    """Return whether junk is the content's latest meaningful classification."""

    if not sha256:
        return False
    row = database.execute(
        "SELECT "
        "MAX(CASE WHEN is_junk = 1 THEN first_seen_at END) AS junk_seen_at, "
        "MAX(CASE WHEN is_junk = 0 AND is_trash = 0 "
        "THEN first_seen_at END) AS protected_seen_at "
        "FROM message_presence WHERE sha256 = ?",
        (sha256,),
    ).fetchone()
    junk_seen_at = None if row is None else row["junk_seen_at"]
    protected_seen_at = None if row is None else row["protected_seen_at"]
    return bool(
        junk_seen_at is not None
        and (
            protected_seen_at is None
            or str(protected_seen_at) <= str(junk_seen_at)
        )
    )
...
def suppress_junk_related_pending_alerts(
    ...
    """Silence queued alerts only while junk is the latest classification."""

    database.execute(
        "UPDATE audit_events SET alerted_at = ?, "
        "last_alert_error = 'suppressed: latest classification is spam/junk' "
        "WHERE alerted_at IS NULL "
        "AND event_type IN ("
        "'unexplained_disappearance', 'trash_observed', 'trash_disappeared'"
        ") AND sha256 IS NOT NULL AND sha256 IN ("
        "SELECT junk.sha256 FROM message_presence AS junk "
        "WHERE junk.is_junk = 1 AND junk.sha256 IS NOT NULL "
        "GROUP BY junk.sha256 HAVING COALESCE(("
        "SELECT MAX(protected.first_seen_at) "
        "FROM message_presence AS protected "
        "WHERE protected.sha256 = junk.sha256 "
        "AND protected.is_junk = 0 AND protected.is_trash = 0"
        "), '') <= MAX(junk.first_seen_at)"
        ")",
        (suppressed_at,),
    )
...
def upsert_presence_folder(
    ...
    scan: MailboxScan,
    observed_at: str,
    promote_existing_to_junk: bool,
) -> None:
    """Record a scanned folder and retain durable junk lineage."""

    if promote_existing_to_junk:
        database.execute(
    ...
    database.execute(
        ...
        "is_junk = MAX(presence_folders.is_junk, excluded.is_junk), "
...
def upsert_message_presences(
    ...
    database.executemany(
        ...
        "is_trash = excluded.is_trash, "
        "is_junk = MAX(message_presence.is_junk, excluded.is_junk), "
        "present = 1, last_seen_at = excluded.last_seen_at, "
        "missing_clean_scans = 0, disappeared_at = NULL",
        (
            (
...
def record_actionable_trash_arrival(
    ...
    if content_is_junk_suppressed(database, sha256):
        return
    record_audit_event(
        database,
...
def selected_response_number(
    client: imaplib.IMAP4_SSL, name: str
) -> int | None:
...
def selected_uidvalidity(client: imaplib.IMAP4_SSL) -> int:
    value = selected_response_number(client, "UIDVALIDITY")
    if value is None:
        raise RuntimeError("Selected mailbox did not return UIDVALIDITY")
    return value


def selected_uidnext(client: imaplib.IMAP4_SSL) -> int | None:
    """Return SELECT's UIDNEXT hint when the server supplied one."""

    return selected_response_number(client, "UIDNEXT")


def load_folder_state(
    database: sqlite3.Connection, folder: str
) -> sqlite3.Row | None:
...
def search_all_uids(
    ...
    if minimum_uid is not None and minimum_uid > MAX_IMAP_UID:
        return []
    arguments: list[str | None] = [None, "SINCE", imap_date]
    if minimum_uid is not None:
        # IMAP UIDs increase monotonically within one UIDVALIDITY namespace.
        # Use the protocol's maximum UID instead of "*" because an N:* range
        # is reversed when N exceeds the mailbox's current highest UID.
        arguments.extend(("UID", f"{minimum_uid}:{MAX_IMAP_UID}"))
    status, data = client.uid("SEARCH", *arguments)
    ...
    if minimum_uid is not None:
        uids = (uid for uid in uids if uid >= minimum_uid)
    return sorted(uids)
...
def archive_mailbox(
    ...
    existing_state = load_folder_state(database, mailbox.key)
    stable_completed_state = (
        incremental_search
        and existing_state is not None
        and int(existing_state["uidvalidity"]) == uidvalidity
        and existing_state["backfill_before_uid"] is not None
        and bool(existing_state["backfill_complete"])
    )
    minimum_uid = (
        int(existing_state["live_cursor_uid"]) + 1
        if stable_completed_state
        else None
    )
    uidnext = selected_uidnext(client) if stable_completed_state else None
    if minimum_uid is not None and uidnext is not None and uidnext <= minimum_uid:
        current_uids = []
    else:
        current_uids = search_all_uids(client, archive_since, minimum_uid)
    state = ensure_folder_state(
        database,
        mailbox.key,
        uidvalidity,
        current_uids,
    ...
    if presence_scans is not None:
        presence_scans.append(
            MailboxScan(
                ...
                complete_snapshot=minimum_uid is None,
                refreshed_uids=tuple(sorted(refreshed_uids)),
            )
        )
...
def reconcile_presence(
    ...
    if any(not scan.complete_snapshot for scan in scans):
        raise ValueError("disappearance reconciliation requires complete UID snapshots")
    ...
    with database:
        database.executemany(
            "UPDATE folder_state SET checked_at = ? "
            "WHERE folder = ? AND uidvalidity = ?",
            (
                (observed_at, scan.folder, scan.uidvalidity)
                for scan in scans
            ),
        )
        previous_folders = {
        ...
        classified_scans: list[MailboxScan] = []

        for scan in scans:
            previous = previous_folders.get(scan.folder)
            if previous is not None and bool(previous["is_junk"]):
                scan = replace(scan, is_junk=True)
            classified_scans.append(scan)
            ...
            upsert_presence_folder(
                ...
                scan,
                observed_at,
                promote_existing_to_junk=(
                    scan.is_junk
                    and (previous is None or not bool(previous["is_junk"]))
                ),
            )
        ...
        for scan in classified_scans:
            archived_by_uid = archived_message_sha256s(
                database,
                scan.folder,
                scan.uidvalidity,
                scan.current_uids,
            ...
            presence_rows: list[tuple[int, str | None]] = []
            for uid in scan.current_uids:
                sha256 = archived_by_uid.get(uid)
                previous = prior_messages.get(uid)
                presence_rows.append((uid, sha256))
                if (
                    baseline_exists
...
def observe_priority_arrivals(
    ...
    baseline_exists = health_value(database, "presence_baseline_complete") == "1"
    trash_arrivals: list[tuple[MailboxScan, int, str | None]] = []
    junk_evidence_changed = False

    with database:
        for scan in scans:
            previous_folder = database.execute(
                "SELECT uidvalidity, is_trash, is_junk, present "
                "FROM presence_folders "
                "WHERE folder = ?",
                (scan.folder,),
            ).fetchone()
            uidvalidity_changed = (
                previous_folder is not None
                and int(previous_folder["uidvalidity"]) != scan.uidvalidity
            )
            previous_is_junk = (
                previous_folder is not None and bool(previous_folder["is_junk"])
            )
            if previous_is_junk:
                scan = replace(scan, is_junk=True)
            # A rapid arrival-only scan must not consume a non-junk UIDVALIDITY
            # reset. The next complete scan records the reset and suppresses
            # per-message deletion conclusions using the old namespace.
            if uidvalidity_changed and not scan.is_junk:
                continue

            folder_reset = previous_folder is None or uidvalidity_changed
            if uidvalidity_changed:
                database.execute(
                    "UPDATE message_presence SET present = 0, "
            ...
            folder_state_changed = (
                ...
                or uidvalidity_changed
                or not bool(previous_folder["present"])
                or bool(previous_folder["is_trash"]) != scan.is_trash
                or (scan.is_junk and not previous_is_junk)
            )
            if folder_state_changed:
                promote_existing_to_junk = (
                    scan.is_junk and not previous_is_junk
                )
                upsert_presence_folder(
                    database,
                    scan,
                    observed_at,
                    promote_existing_to_junk=promote_existing_to_junk,
                )
                junk_evidence_changed |= promote_existing_to_junk

            candidate_uids = tuple(
                sorted(set(scan.current_uids) | set(scan.refreshed_uids))
            )
            ...
            prior_uid_parameters: tuple[str | int, ...] = (
                scan.folder,
                scan.uidvalidity,
            )
            if not scan.complete_snapshot:
                prior_uid_query += " AND uid >= ?"
                prior_uid_parameters += (min(candidate_uids),)
            prior_uids = {
                int(row["uid"])
                for row in database.execute(
                    prior_uid_query,
                    prior_uid_parameters,
                ).fetchall()
            }
            refreshed_uids = set(scan.refreshed_uids)
            presence_rows: list[tuple[int, str | None]] = []
            for uid in candidate_uids:
                previously_recorded = uid in prior_uids
                if previously_recorded and uid not in refreshed_uids:
                    continue
            ...
            if presence_rows:
                ...
                junk_evidence_changed |= scan.is_junk and any(
                    sha256 is not None for _uid, sha256 in presence_rows
                )
...
def archive_cycle(
    ...
    start_log = LOG.info if full_scan else LOG.debug
    start_log(
    ...
    for mailbox in mailboxes:
        ...
        try:
            attempted, uploaded = archive_mailbox(
                ...
                destination_id,
                presence_scans,
                incremental_search=not full_scan,
            )
    ...
    completion_log = (
        LOG.info
        if full_scan or total_attempted or total_uploaded
        else LOG.debug
    )
    completion_log(
